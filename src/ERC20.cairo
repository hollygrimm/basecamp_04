...
mod tests {
    ...
    fn test_01_constructor() {
        ...
        assert(res_total_supply == initial_supply, 'Tot supply does not match');

        let res_balance_account = ERC20::balance_of(account);
        assert(res_balance_account == initial_supply, 'Acct balance does not match');
    }
}
